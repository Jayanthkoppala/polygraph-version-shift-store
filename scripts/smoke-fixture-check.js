...
const expectedKeys = ['anchors', 'cache', 'canonical_source', 'commits', 'contract_version', 'generation', 'generator', 'html_sha256', 'invariants', 'mission_id', 'parent_generation', 'published_at', 'schema_version', 'seed', 'template_sha256', 'version'];
...
if (manifest.schema_version !== 2 || manifest.contract_version !== 'polygraph-owned-product/v1') fail('manifest schema and product contract versions are invalid');
...
if (manifest.generator?.name !== 'evolve-store' || manifest.generator?.version !== 1) fail('generator identity is invalid');
